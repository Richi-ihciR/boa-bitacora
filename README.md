...
# -*- coding: utf-8 -*-
"""
B.O.A.2.5.4 - Bitácora de Obras Artísticas
Versión Completa: Bitácora + Calendario + Canvas + Fotos
...
class BOABitacora:
    def __init__(self):
        # Ventana principal
        self.root = tk.Tk()
        self.root.title("B.O.A.2.5.4 - Bitácora de Obras Artísticas")
        self.root.geometry("1200x800")
        self.root.configure(bg='#f0f0f0')
        
        # Datos
        self.entradas = []  # Lista de entradas de bitácora
    ...
    def crear_interfaz(self):
        ...
        self.crear_pestaña_bitacora()
        self.crear_pestaña_calendario()
        self.crear_pestaña_canvas()
        self.crear_pestaña_galeria()
        self.crear_pestaña_busqueda()
        
        # Bindear evento de cambio de pestaña
    ...
    def crear_pestaña_dashboard(self):
        ...
        title_label = ttk.Label(
            dashboard_frame,
            text="🎨 B.O.A.2.5.4 - Bitácora de Obras Artísticas",
            style='Title.TLabel'
        )
        title_label.pack(pady=20)
        
        # Frame de estadísticas
        stats_frame = ttk.LabelFrame(dashboard_frame, text="📊 Estadísticas", padding=20)
        ...
        # Mostrar estadísticas en grid
        stats_grid = ttk.Frame(stats_frame)
        stats_grid.pack(fill="x")
        
        # Fila 1
        ttk.Label(stats_grid, text="📝 Entradas de Bitácora:", style='Info.TLabel').grid(row=0, column=0, sticky="w", padx=10)
        ttk.Label(stats_grid, text=str(total_entradas), font=('Arial', 12, 'bold')).grid(row=0, column=1, sticky="w")
        
        ttk.Label(stats_grid, text="🎨 Entradas con Obras:", style='Info.TLabel').grid(row=0, column=2, sticky="w", padx=10)
        ttk.Label(stats_grid, text=str(entradas_con_obras), font=('Arial', 12, 'bold')).grid(row=0, column=3, sticky="w")
        
        # Fila 2
        ttk.Label(stats_grid, text="🖼️ Total de Obras:", style='Info.TLabel').grid(row=1, column=0, sticky="w", padx=10)
        ttk.Label(stats_grid, text=str(total_obras), font=('Arial', 12, 'bold')).grid(row=1, column=1, sticky="w")
        
        ttk.Label(stats_grid, text="🖌️ Dibujos Digitales:", style='Info.TLabel').grid(row=1, column=2, sticky="w", padx=10)
        ttk.Label(stats_grid, text=str(dibujos), font=('Arial', 12, 'bold')).grid(row=1, column=3, sticky="w")
        
        # Fila 3
        ttk.Label(stats_grid, text="📷 Fotos de Obras:", style='Info.TLabel').grid(row=2, column=0, sticky="w", padx=10)
        ...
        if entrada_hoy:
            ttk.Label(stats_grid, text="📅 Entrada de Hoy:", style='Info.TLabel').grid(row=2, column=2, sticky="w", padx=10)
            ttk.Label(stats_grid, text="✅ Registrada", font=('Arial', 10, 'bold'), foreground='green').grid(row=2, column=3, sticky="w")
        else:
            ttk.Label(stats_grid, text="📅 Entrada de Hoy:", style='Info.TLabel').grid(row=2, column=2, sticky="w", padx=10)
            ttk.Label(stats_grid, text="❌ Pendiente", font=('Arial', 10, 'bold'), foreground='red').grid(row=2, column=3, sticky="w")
        
        # Frame de acciones rápidas
        acciones_frame = ttk.LabelFrame(dashboard_frame, text="🚀 Acciones Rápidas", padding=20)
        ...
        ttk.Button(
            botones_frame,
            text="📝 Nueva Entrada de Bitácora",
        ...
        ttk.Button(
            botones_frame,
            text="🎨 Crear Dibujo Digital",
            command=lambda: self.cambiar_a_pestaña("Canvas"),
            width=25
        ).grid(row=0, column=1, padx=10, pady=5)
        
        ttk.Button(
            botones_frame,
            text="📷 Subir Foto de Obra",
        ...
        ttk.Button(
            botones_frame,
            text="📅 Ver Calendario",
            command=lambda: self.cambiar_a_pestaña("Calendario"),
            width=25
        ).grid(row=1, column=1, padx=10, pady=5)
        
        # Últimas entradas
        recientes_frame = ttk.LabelFrame(dashboard_frame, text="📖 Últimas Entradas", padding=10)
        recientes_frame.pack(fill="both", expand=True, padx=20, pady=10)
        
        self.actualizar_recientes(recientes_frame)
    
    def crear_pestaña_bitacora(self):
        """Crear pestaña de bitácora"""
        bitacora_frame = ttk.Frame(self.notebook)
        self.notebook.add(bitacora_frame, text="📝 Bitácora")
        
        # Frame superior con fecha y controles
        top_frame = ttk.Frame(bitacora_frame)
        top_frame.pack(fill="x", padx=10, pady=10)
        
        ttk.Label(top_frame, text="📅 Fecha:", style='Subtitle.TLabel').pack(side="left")
        ...
        # Frame del editor de bitácora
        editor_frame = ttk.LabelFrame(bitacora_frame, text="✍️ Entrada de Bitácora", padding=10)
        ...
        ttk.Label(titulo_frame, text="Título:").pack(side="left")
        ...
        # Campo de categoría/estado de ánimo
        meta_frame = ttk.Frame(editor_frame)
        meta_frame.pack(fill="x", pady=5)
        
        ttk.Label(meta_frame, text="Categoría:").pack(side="left")
        self.categoria_var = tk.StringVar()
        self.categoria_combo = ttk.Combobox(
            meta_frame, 
            textvariable=self.categoria_var,
            values=["Inspiración", "Práctica", "Reflexión", "Proyecto", "Experimento", "Estudio"],
            width=15
        )
        self.categoria_combo.pack(side="left", padx=10)
        
        ttk.Label(meta_frame, text="Estado de Ánimo:").pack(side="left", padx=(20,5))
        self.animo_var = tk.StringVar()
        self.animo_combo = ttk.Combobox(
            meta_frame,
            textvariable=self.animo_var,
            values=["😊 Feliz", "🤔 Pensativo", "😤 Frustrado", "🤩 Inspirado", "😴 Cansado", "🔥 Motivado"],
            width=15
        )
        ...
        obras_frame = ttk.LabelFrame(editor_frame, text="🎨 Obras Asociadas a Esta Entrada", padding=10)
        obras_frame.pack(fill="x", pady=10)
        
        # Botones para añadir obras
    ...
    def crear_pestaña_calendario(self):
        ...
        ttk.Label(control_frame, text="📅 Navegación:", style='Subtitle.TLabel').pack(side="left")
        
        self.mes_var = tk.StringVar(value=str(self.fecha_seleccionada.month))
        self.año_var = tk.StringVar(value=str(self.fecha_seleccionada.year))
        
        ttk.Button(control_frame, text="◀", command=self.mes_anterior).pack(side="left", padx=5)
        ...
        año_combo.bind("<<ComboboxSelected>>", self.actualizar_calendario)
        
        ttk.Button(control_frame, text="▶", command=self.mes_siguiente).pack(side="left", padx=5)
        ttk.Button(control_frame, text="Hoy", command=self.ir_a_hoy_calendario).pack(side="left", padx=20)
        
        # Frame del calendario
        self.calendario_frame = ttk.LabelFrame(calendario_frame, text="📅 Calendario", padding=10)
        self.calendario_frame.pack(fill="both", expand=True, padx=10, pady=10)
        
        self.crear_widget_calendario()
        
        # Frame de información del día seleccionado
        self.info_dia_frame = ttk.LabelFrame(calendario_frame, text="📝 Información del Día", padding=10)
        self.info_dia_frame.pack(fill="x", padx=10, pady=5)
        
        self.actualizar_info_dia()
    
    def crear_pestaña_canvas(self):
        """Crear pestaña de canvas de dibujo"""
        canvas_frame = ttk.Frame(self.notebook)
        self.notebook.add(canvas_frame, text="🎨 Canvas")
        
        # Frame de herramientas
        tools_frame = ttk.LabelFrame(canvas_frame, text="🛠️ Herramientas", padding=10)
        ...
        ttk.Radiobutton(tools_row1, text="✏️ Lápiz", variable=self.tool_var, value="pen").pack(side="left", padx=10)
        ttk.Radiobutton(tools_row1, text="🧽 Borrador", variable=self.tool_var, value="eraser").pack(side="left", padx=10)
        ...
        # Botones de acción
        ttk.Button(tools_row2, text="🗑️ Limpiar", command=self.limpiar_canvas).pack(side="right", padx=5)
        ttk.Button(tools_row2, text="💾 Guardar Dibujo", command=self.guardar_dibujo).pack(side="right", padx=5)
        
        # Canvas de dibujo
        canvas_container = ttk.LabelFrame(canvas_frame, text="🎨 Área de Dibujo", padding=10)
    ...
    def crear_pestaña_galeria(self):
        """Crear pestaña de galería"""
        galeria_frame = ttk.Frame(self.notebook)
        self.notebook.add(galeria_frame, text="🖼️ Galería")
        
        # Frame de controles
        controles_frame = ttk.Frame(galeria_frame)
        controles_frame.pack(fill="x", padx=10, pady=10)
        
        ttk.Label(controles_frame, text="🖼️ Galería de Obras", style='Title.TLabel').pack(side="left")
        
        ttk.Button(controles_frame, text="🔄 Actualizar", command=self.actualizar_galeria).pack(side="right", padx=5)
        ttk.Button(controles_frame, text="💾 Exportar", command=self.exportar_datos).pack(side="right", padx=5)
        
        # Frame con scroll para la galería
    ...
    def crear_pestaña_busqueda(self):
        ...
        # Frame de búsqueda
        search_frame = ttk.LabelFrame(busqueda_frame, text="🔍 Buscar en Bitácora", padding=10)
        ...
        ttk.Button(search_controls, text="🔍 Buscar", command=self.realizar_busqueda).pack(side="left", padx=5)
        ttk.Button(search_controls, text="🗑️ Limpiar", command=self.limpiar_busqueda).pack(side="left", padx=5)
        ...
        ttk.Checkbutton(opciones_frame, text="En títulos", variable=self.buscar_titulo).pack(side="left", padx=10)
        ttk.Checkbutton(opciones_frame, text="En contenido", variable=self.buscar_contenido).pack(side="left", padx=10)
        ttk.Checkbutton(opciones_frame, text="En categorías", variable=self.buscar_categoria).pack(side="left", padx=10)
        
        # Frame de resultados
        self.resultados_frame = ttk.LabelFrame(busqueda_frame, text="📋 Resultados", padding=10)
    ...
    def actualizar_recientes(self, parent):
        ...
        if not self.entradas:
            ttk.Label(parent, text="No hay entradas aún. ¡Crea tu primera entrada!").pack(pady=20)
            return
        ...
        for entrada in entradas_ordenadas:
            ...
            ttk.Label(
                entrada_frame,
                text=f"📅 {fecha_str} - {entrada.get('titulo', 'Sin título')}",
                font=('Arial', 10, 'bold')
            ).pack(side="left")
            ...
            if entrada.get('obras'):
                ttk.Label(
                    entrada_frame,
                    text=f"🎨 {len(entrada['obras'])} obra(s)",
                    font=('Arial', 9),
                    foreground='blue'
                ).pack(side="right")
    ...
    def nueva_entrada_hoy(self):
        ...
        self.fecha_seleccionada = datetime.date.today()
        self.cambiar_a_pestaña("Bitácora")
        self.cargar_entrada_actual()
    
    def cambiar_a_pestaña(self, nombre_pestaña):
        ...
        if nombre_pestaña in pestañas:
            self.notebook.select(pestañas[nombre_pestaña])
    ...
    def actualizar_lista_obras_entrada(self):
        ...
        for i, obra in enumerate(entrada['obras']):
            ...
            tipo_emoji = "🎨" if obra['tipo'] == 'dibujo' else "📷"
            ttk.Label(
                obra_frame,
    ...
    obra_frame,
    text="👁️ Ver",
    command=lambda o=obra: self.ver_detalle_obra(o)
    ...
    def crear_widget_calendario(self):
        ...
        # Título del mes
        titulo_mes = f"{calendar.month_name[mes]} {año}"
        ...
        for semana_num, semana in enumerate(cal):
            for dia_num, dia in enumerate(semana):
                ...
                fecha_dia = datetime.date(año, mes, dia)
    ...
    def actualizar_info_dia(self):
        ...
        ttk.Label(self.info_dia_frame, text=f"📅 {fecha_str}", style='Subtitle.TLabel').pack(anchor="w")
        
        entrada = self.obtener_entrada_fecha(self.fecha_seleccionada)
        if entrada:
            ttk.Label(self.info_dia_frame, text=f"📝 {entrada.get('titulo', 'Sin título')}", font=('Arial', 10, 'bold')).pack(anchor="w", pady=2)
            ttk.Label(self.info_dia_frame, text=f"📂 {entrada.get('categoria', 'Sin categoría')}", font=('Arial', 9)).pack(anchor="w")
            ttk.Label(self.info_dia_frame, text=f"{entrada.get('animo', 'Sin estado de ánimo')}", font=('Arial', 9)).pack(anchor="w")
            
            if entrada.get('obras'):
                ttk.Label(self.info_dia_frame, text=f"🎨 {len(entrada['obras'])} obra(s) asociada(s)", font=('Arial', 9), foreground='blue').pack(anchor="w")
            
            # Botón para editar
            ttk.Button(
                self.info_dia_frame,
                text="✏️ Editar Entrada",
                command=lambda: [self.cambiar_a_pestaña("Bitácora"), self.cargar_entrada_actual()]
            ).pack(anchor="w", pady=5)
        else:
            ttk.Label(self.info_dia_frame, text="Sin entrada para este día").pack(anchor="w", pady=2)
            ttk.Button(
                self.info_dia_frame,
                text="➕ Crear Entrada",
                command=lambda: [self.cambiar_a_pestaña("Bitácora"), self.cargar_entrada_actual()]
    ...
    def mes_anterior(self):
        ...
        año_actual = int(self.año_var.get())
        ...
        self.año_var.set(str(nuevo_año))
        self.crear_widget_calendario()
    ...
    def ir_a_hoy_calendario(self):
        ...
        self.año_var.set(str(hoy.year))
        self.fecha_seleccionada = hoy
        self.crear_widget_calendario()
        self.actualizar_info_dia()
    
    def actualizar_calendario(self, event=None):
        """Actualizar el calendario cuando cambian mes/año"""
        self.crear_widget_calendario()
    
    # Métodos para el canvas
    ...
    def guardar_dibujo(self):
        ...
        titulo = simpledialog.askstring("Título", "Título del dibujo:")
        if not titulo:
            return
        ...
        asociar = messagebox.askyesno(
            "Asociar a Entrada", 
            f"¿Deseas asociar este dibujo a la entrada de bitácora de hoy ({datetime.date.today().strftime('%d/%m/%Y')})?"
        )
            ...
            if asociar:
                ...
                if entrada_hoy:
                    ...
                    nueva_entrada = {
                        ...
                        'categoria': 'Práctica',
                        'animo': '🎨 Creativo',
                        'contenido': f'Creé un dibujo digital: "{titulo}"',
                        'obras': [obra]
                    }
                    self.entradas.append(nueva_entrada)
            
            self.guardar_datos()
            messagebox.showinfo("Éxito", f"Dibujo '{titulo}' guardado correctamente")
    ...
    def subir_foto_para_entrada(self):
        ...
        titulo = simpledialog.askstring("Título", "Título de la obra:")
        if not titulo:
            return
        
        descripcion = simpledialog.askstring("Descripción", "Descripción de la obra (opcional):")
        categoria = simpledialog.askstring("Categoría", "Categoría (pintura, escultura, dibujo, etc.):")
        ...
        try:
            ...
            obra = {
                ...
                'categoria': categoria or 'Obra artística',
                'tipo': 'foto',
                'archivo': nombre_archivo,
                'fecha_creacion': datetime.datetime.now().isoformat(),
            ...
            if hasattr(self, 'fecha_seleccionada'):
                ...
                asociar = messagebox.askyesno(
                    "Asociar a Entrada",
                    f"¿Deseas asociar esta foto a la entrada del {fecha_str}?"
                )
                ...
                if asociar:
                    ...
                    if entrada:
                        ...
                        nueva_entrada = {
                            ...
                            'animo': '📷 Documentando',
                            'contenido': f'Subí una foto de mi obra: "{titulo}"',
                            'obras': [obra]
                        }
                        self.entradas.append(nueva_entrada)
                        self.actualizar_lista_obras_entrada()
            ...
            self.guardar_datos()
            messagebox.showinfo("Éxito", f"Foto '{titulo}' subida correctamente")
    ...
    def crear_dibujo_para_entrada(self):
        ...
        self.cambiar_a_pestaña("Canvas")
    
    def eliminar_obra_entrada(self, indice):
        """Eliminar obra de la entrada actual"""
        if messagebox.askyesno("Confirmar", "¿Eliminar esta obra de la entrada?"):
            entrada = self.obtener_entrada_fecha(self.fecha_seleccionada)
    ...
    def actualizar_galeria(self):
        """Actualizar la galería de obras"""
        # Limpiar frame
        ...
        if not todas_las_obras:
            ttk.Label(self.galeria_scroll_frame, text="No hay obras en la galería").pack(pady=50)
            return
    ...
    def crear_tarjeta_obra_galeria(self, parent, obra, index):
        """Crear tarjeta de obra para la galería"""
        # Frame de la tarjeta
        card_frame = ttk.LabelFrame(parent, text=f"🎨 {obra['titulo']}", padding=10)
    ...
    command=lambda o=obra: self.ver_detalle_obra(o)
        ...
        ttk.Label(
            info_frame,
            text=f"📅 Creado: {fecha_creacion.strftime('%d/%m/%Y %H:%M')}",
            font=('Arial', 9)
        ).pack(anchor="w")
        ...
        if obra['tipo'] == 'dibujo':
            ttk.Label(
                info_frame,
                text=f"🎨 Color: {obra.get('color_principal', 'N/A')} | Grosor: {obra.get('grosor_pincel', 'N/A')}px",
                font=('Arial', 8)
            ).pack(anchor="w")
        elif obra['tipo'] == 'foto':
            if obra.get('categoria'):
                ttk.Label(
                    info_frame,
                    text=f"📂 Categoría: {obra['categoria']}",
                    font=('Arial', 8)
                ).pack(anchor="w")
    ...
    def ver_detalle_obra(self, obra):
        """Mostrar detalles e imagen de una obra en ventana emergente"""
        detalle_win = tk.Toplevel(self.root)
        detalle_win.title(f"Detalle - {obra['titulo']}")
        detalle_win.geometry("600x600")

        # Título
        ttk.Label(detalle_win, text=obra['titulo'], font=('Arial', 14, 'bold')).pack(pady=10)

        # Info básica
        ttk.Label(detalle_win, text=f"Tipo: {obra['tipo']}").pack()
        ttk.Label(detalle_win, text=f"Fecha: {obra['fecha_creacion']}").pack()
        if obra.get('descripcion'):
            ttk.Label(detalle_win, text=f"Descripción: {obra['descripcion']}").pack()

        # Mostrar imagen si aplica
        ruta = os.path.join(self.carpeta_imagenes, obra['archivo'])
        if obra['tipo'] in ["foto", "dibujo"] and os.path.exists(ruta):
            try:
                img = Image.open(ruta)
                img.thumbnail((500, 400))
                img_tk = ImageTk.PhotoImage(img)
                lbl_img = ttk.Label(detalle_win, image=img_tk)
                lbl_img.image = img_tk  # mantener referencia
                lbl_img.pack(pady=10)
            except Exception as e:
                ttk.Label(detalle_win, text=f"No se pudo cargar la imagen: {e}").pack()

    
    # Métodos para búsqueda
    def realizar_busqueda(self):
        """Realizar búsqueda en la bitácora"""
        ...
        for entrada in self.entradas:
            ...
            if self.buscar_categoria.get() and termino in entrada.get('categoria', '').lower():
                coincidencias.append("categoría")
        ...
        if not resultados:
            ...
            for resultado in resultados:
                ...
                self.resultados_text.insert(tk.END, f"📅 {fecha_str} - {resultado.get('titulo', 'Sin título')}\n")
                self.resultados_text.insert(tk.END, f"   Coincidencias en: {', '.join(resultado['coincidencias'])}\n")
                
                if resultado.get('categoria'):
                    self.resultados_text.insert(tk.END, f"   📂 Categoría: {resultado['categoria']}\n")
                
                if resultado.get('obras'):
                    self.resultados_text.insert(tk.END, f"   🎨 {len(resultado['obras'])} obra(s) asociada(s)\n")
                ...
                if contenido:
                    self.resultados_text.insert(tk.END, f"   📝 {contenido}\n")
    ...
    def on_tab_change(self, event):
        ...
        pestaña_seleccionada = event.widget.tab('current')['text']
        
        # Actualizar vista según pestaña
        if "Dashboard" in pestaña_seleccionada:
            self.vista_actual = "dashboard"
        ...
        elif "Galería" in pestaña_seleccionada:
            self.vista_actual = "galeria"
            self.actualizar_galeria()
        
        elif "Calendario" in pestaña_seleccionada:
            self.vista_actual = "calendario"
            self.crear_widget_calendario()
            self.actualizar_info_dia()
        
        elif "Bitácora" in pestaña_seleccionada:
